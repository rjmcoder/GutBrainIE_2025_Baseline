...
def eval_submission_6_1_NER(path):
    ...
    # print the total number of annotated entities per label, and the total across all labels
    print("\n=== Start: Annotated entities per label ===\n")
    for label in sorted(count_annotated_entities_per_label, key=count_annotated_entities_per_label.get, reverse=True):
        print(f"Label: {label}, Annotated entities: {count_annotated_entities_per_label[label]}")
    total_annotated = sum(count_annotated_entities_per_label.values())
    print(f"\nTotal annotated entities: {total_annotated}")
    print("\n=== End: Annotated entities per label ===\n")
    ...
    # count the number of incorrect entities per label
    count_incorrect_per_label = {label: 0 for label in list(count_annotated_entities_per_label.keys())}
    ...
    for label in sorted(count_incorrect_per_label, key=count_incorrect_per_label.get, reverse=True):
        print(f"Label: {label}, Incorrect predictions: {count_incorrect_per_label[label]} ({round(count_incorrect_per_label[label]/count_annotated_entities_per_label[label]*100, 2)}%), Correct predictions: {count_true_positives_per_label[label]} ({round(count_true_positives_per_label[label]/count_predicted_entities_per_label[label]*100, 2)}%)")

    # Print the total number of incorrect predictions and correct predictions across all labels
    total_incorrect = sum(count_incorrect_per_label.values())
    total_correct = sum(count_true_positives_per_label.values())
    print(f"Total incorrect predictions: {total_incorrect}, Total correct predictions: {total_correct}")
